...
public class MapServiceImpl implements MapService {
    ...
    private final String key = System.getenv("TENCENT_MAP_KEY");
    ...
    public DrivingLineVo calculateDrivingLine(CalculateDrivingLineForm calculateDrivingLineForm) {
        ...
        // 压测专用：直接返回模拟数据，不调用腾讯地图
        DrivingLineVo drivingLineVo = new DrivingLineVo();
        drivingLineVo.setDuration(new BigDecimal("300"));     // 5分钟
        drivingLineVo.setDistance(new BigDecimal("2.50"));   // 2.5公里
        drivingLineVo.setPolyline(null);
        return drivingLineVo;
